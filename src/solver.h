...
public:
    ...
    Matrix SolveAdaptive(const Matrix& matrix_c, const Matrix& matrix_a, const Matrix& matrix_lb, const Matrix& matrix_ub
        ...
        while (true) {
            ...
            if (opora_dim > 0) {
                u.UpdateMatrix(a_op_inv.GetTransposed() * matrix_c.GetSubMatrixByRow(basis_j), basis_i, {0});
            ...
            for (auto idx : basis_i) {
                ...
                if (EqualNotStrict(u(idx, 0), 0) && z(idx, 0) < matrix_ub(idx, 0) && z(idx, 0) > matrix_lb(idx, 0)) {
                    continue;
                }
            ...
            for (auto jdx : basis_j_residual) {
                ...
                if (EqualNotStrict(delta(jdx, 0), 0) && x(jdx, 0) < matrix_ud(jdx, 0) && x(jdx, 0) > matrix_ld(jdx, 0)) {
                    continue;
                }
            ...
            if (opora_dim > 0) {
                l.UpdateMatrix(a_op_inv * omega.GetSubMatrixByRow(basis_j) - a_op_inv * matrix_a.GetSubMatrix(basis_i, basis_j_residual) * l.GetSubMatrixByRow(basis_j_residual), basis_j, {0});
            }
            ...
            if (bad_index_type == 0) {
                ...
                if (opora_dim > 0) {
                    matrix_j_res.UpdateMatrix(a_op_inv * matrix_a.GetSubMatrix(basis_i, basis_j_residual), basis_i, basis_j_residual);
                    matrix_i.UpdateMatrix(-1 * a_op_inv, basis_i, basis_j);
                    
                    ksi_j.UpdateMatrix(matrix_j_res.GetSubMatrixByRow({bad_index}).GetTransposed() * sign, basis_j_residual, {0});
                    ksi_i.UpdateMatrix(matrix_i.GetSubMatrixByRow({bad_index}).GetTransposed(), basis_i, {0});
                }
                ...
                int sign = ((matrix_a.GetSubMatrixByRow({bad_index}) * x) == matrix_ub.GetSubMatrixByRow({bad_index})) ? 1 : -1;
                if (opora_dim > 0) {
                    matrix_j_res.UpdateMatrix(
                        matrix_a.GetSubMatrix(basis_i_residual, basis_j_residual) - matrix_a.GetSubMatrix(basis_i_residual, basis_j) * a_op_inv * matrix_a.GetSubMatrix(basis_i, basis_j_residual)
                      , basis_i_residual, basis_j_residual);
                    matrix_i.UpdateMatrix(
                        matrix_a.GetSubMatrix(basis_i_residual, basis_j) * a_op_inv
                      , basis_i_residual, basis_j);

                    ksi_j.UpdateMatrix(matrix_j_res.GetSubMatrixByRow({bad_index}).GetTransposed().GetSubMatrixByRow(basis_j_residual) * sign, basis_j_residual, {0});
                    ksi_i.UpdateMatrix(matrix_i.GetSubMatrixByRow({bad_index}).GetTransposed().GetSubMatrixByRow(basis_j) * sign, basis_i, {0});
                } else {
                    // matrix_j_res.UpdateMatrix(matrix_a.GetSubMatrix(basis_i_residual, basis_j_residual), basis_i_residual, basis_j_residual);
            ...
            kappa_j.UpdateMatrix(x.GetSubMatrixByRow(basis_j_residual) + l.GetSubMatrixByRow(basis_j_residual), basis_j_residual, {0});
            if (opora_dim > 0) {
